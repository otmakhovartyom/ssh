...
{
	static char* default_path = "/home/otard/Progs/SSH_project/src/daemon_logs";
...
int send_file(char* input, int client_fd, int size, int is_udp, struct sockaddr_in* server, int key)
{
	...
	*strchr(path_name, ' ') = '\0';
	...
	struct stat stats;
	int stat_error = stat(path_name, &stats);
...
//	log_info("file %s\n", buf);
	...
	if (sendto(client_fd, "ready to get", strlen("ready to get"), 0, (struct sockaddr*) server, sizeof(*server)) < 0)
	{
